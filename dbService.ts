  ...
  return new Promise((resolve, reject) => {
    ...
    request.onupgradeneeded = (event) => {
      ...
      if (!dbInstance.objectStoreNames.contains(SCAR_STORE_NAME)) {
        const scarObjectStore = dbInstance.createObjectStore(SCAR_STORE_NAME, { keyPath: 'id', autoIncrement: true });
        scarObjectStore.createIndex('createdAt', 'createdAt', { unique: false });
      }
      if (!dbInstance.objectStoreNames.contains(ACNE_STORE_NAME)) {
        const acneObjectStore = dbInstance.createObjectStore(ACNE_STORE_NAME, { keyPath: 'id', autoIncrement: true });
      ...
      if (!dbInstance.objectStoreNames.contains(REJUVENATION_STORE_NAME)) {
        const rejuvenationObjectStore = dbInstance.createObjectStore(REJUVENATION_STORE_NAME, { keyPath: 'id', autoIncrement: true });
