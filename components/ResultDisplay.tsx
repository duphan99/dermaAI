...
import type { AnalysisResult, TreatmentPlan, InClinicProcedure, MachineInfo, CosmeticInfo, ExpertAdvice } from '../types';
...
interface ResultDisplayProps {
    ...
    editableData: { treatmentPlan: TreatmentPlan, expertAdvice: ExpertAdvice };
    onDataChange: (data: { treatmentPlan: TreatmentPlan, expertAdvice: ExpertAdvice }) => void;
...
export const ResultDisplay: React.FC<ResultDisplayProps> = ({ result, editableData, onDataChange, availableMachines, availableCosmetics }) => {
  const { diagnosis, disclaimer } = result;
  const { treatmentPlan, expertAdvice } = editableData;
  ...
  const handlePlanChange = (newPlan: TreatmentPlan) => {
    onDataChange({ ...editableData, treatmentPlan: newPlan });
  };

  const handleExpertAdviceChange = (newAdvice: ExpertAdvice) => {
    onDataChange({ ...editableData, expertAdvice: newAdvice });
  };
  ...
  return (
    <div className="animate-fade-in space-y-6">
      ...
      <div className="p-5 bg-white border border-slate-200 rounded-2xl">
        <h3 className="text-lg md:text-xl font-bold text-slate-800 mb-4">Chẩn đoán của AI</h3>
        <div className="space-y-3">
      ...
      {/* Illustrative Images Card */}
      <IllustrativeImages condition={diagnosis.condition} />

      {/* Treatment Plan Card */}
       <div className="p-5 bg-white border border-slate-200 rounded-2xl">
        <h3 className="text-lg md:text-xl font-bold text-slate-800 mb-4">Phác đồ điều trị (Có thể chỉnh sửa và bổ sung)</h3>
        <div className="space-y-3">
           <AccordionSection
            ...
            >
              <EditableRoutine
                  routine={treatmentPlan.morningRoutine}
            ...
            >
              <EditableRoutine
                  routine={treatmentPlan.eveningRoutine}
            ...
            >
               <EditableProcedures
                  procedures={treatmentPlan.inClinicProcedures}
                  onPlanChange={handlePlanChange}
                  currentPlan={treatmentPlan}
              />
            </AccordionSection>
      ...
      <div className="p-5 bg-white border border-slate-200 rounded-2xl">
        ...
        <div className="space-y-3">
          <AccordionSection
            ...
            >
              <EditableList
                  items={expertAdvice.recommendations}
                  onItemsChange={(newItems) => handleExpertAdviceChange({ ...expertAdvice, recommendations: newItems })}
                  placeholder="Thêm khuyến nghị..."
              />
            </AccordionSection>

            <AccordionSection
              title="Liệu pháp kết hợp"
            ...
            >
              <EditableList
                  items={expertAdvice.combinedTreatments}
                  onItemsChange={(newItems) => handleExpertAdviceChange({ ...expertAdvice, combinedTreatments: newItems })}
                  placeholder="Thêm liệu pháp kết hợp..."
              />
            </AccordionSection>
        </div>
      </div>
