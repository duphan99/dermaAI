import type { PatientRecord, InClinicProcedure, ScarPatientRecord, ScarInfo, AcnePatientRecord, RejuvenationPatientRecord, MelasmaPatientRecord } from '../types';
...
const addBulletedStyledText = (doc: any, text: string, x: number, y: number, maxWidth: number) => {
    const bullet = "• ";
    const bulletWidth = doc.getTextWidth(bullet);
    doc.text(bullet, x - 2, y); // Draw bullet

    const textX = x + bulletWidth - 2;
    ...
    if (parts.length > 1 && parts[0].trim().length > 0) {
        ...
        if (labelWidth < textMaxWidth) {
            doc.text(label, textX, y);
            doc.setFont('BeVietnamPro', 'normal');
            const valueLines = doc.splitTextToSize(value, textMaxWidth - labelWidth);
            doc.text(valueLines, textX + labelWidth, y);
            return valueLines.length * 5;
        } else {
            // If label is too long, render everything as normal text.
            doc.setFont('BeVietnamPro', 'normal');
            const fullTextLines = doc.splitTextToSize(text, textMaxWidth);
            doc.text(fullTextLines, textX, y);
            return fullTextLines.length * 5;
        }
    } else {
        doc.setFont('BeVietnamPro', 'normal');
...
export const generatePatientReportPDF = async (record: Omit<PatientRecord, 'id' | 'createdAt'>, disclaimer: string): Promise<void> => {
    ...
    const addProcedureRows = (title: string, items: InClinicProcedure[]) => {
      if (items.length > 0) {
        const firstProcContent = `${items[0].name}\n(Tần suất: ${items[0].frequency})\n${items[0].description}`;
        tableBody.push([
          { 
            content: title, 
            rowSpan: items.length, 
            styles: { valign: 'middle', halign: 'center', font: 'BeVietnamPro', fontStyle: 'bold' } 
          },
          firstProcContent
        ]);
        ...
        for (let i = 1; i < items.length; i++) {
          const proc = items[i];
          const procContent = `${proc.name}\n(Tần suất: ${proc.frequency})\n${proc.description}`;
          tableBody.push([procContent]);
        }
      }
    };
    ...
    currentY = addSectionTitle(doc, "4. Lời khuyên từ Chuyên gia", currentY > 240 ? 42 : currentY);

    if (record.expertAdvice.recommendations.length > 0) {
        doc.setFont('BeVietnamPro', 'bold');
        doc.setFontSize(11);
        doc.text("Khuyến nghị chung:", PAGE_MARGIN, currentY);
        currentY += 6;
        
        record.expertAdvice.recommendations.forEach(item => {
            const itemHeight = addBulletedStyledText(doc, item, PAGE_MARGIN + 4, currentY, 178);
            if (currentY + itemHeight > 270) {
                doc.addPage();
                currentY = 42;
                addBulletedStyledText(doc, item, PAGE_MARGIN + 4, currentY, 178);
            }
    ...
    if (record.expertAdvice.combinedTreatments.length > 0) {
        doc.setFont('BeVietnamPro', 'bold');
        doc.setFontSize(11);
        doc.text("Liệu pháp kết hợp:", PAGE_MARGIN, currentY);
        currentY += 6;
        
        record.expertAdvice.combinedTreatments.forEach(item => {
            const itemHeight = addBulletedStyledText(doc, item, PAGE_MARGIN + 4, currentY, 178);
            if (currentY + itemHeight > 270) {
                doc.addPage();
                currentY = 42;
...
export const generateScarReportPDF = async (record: Omit<ScarPatientRecord, 'id' | 'createdAt'>): Promise<void> => {
    ...
    if (inClinicProcedures.length > 0) {
        const firstProcContent = `${inClinicProcedures[0].name}\n(Tần suất: ${inClinicProcedures[0].frequency})\n${inClinicProcedures[0].description}`;
        tableBody.push([
            { content: 'Tại phòng khám', rowSpan: inClinicProcedures.length, styles: { valign: 'middle', halign: 'center', font: 'BeVietnamPro', fontStyle: 'bold' } },
            firstProcContent
        ]);
        for (let i = 1; i < inClinicProcedures.length; i++) {
            const proc = inClinicProcedures[i];
            const procContent = `${proc.name}\n(Tần suất: ${proc.frequency})\n${proc.description}`;
            tableBody.push([procContent]);
        }
    }
    ...
    // --- Expert Advice ---
    const { expertAdvice } = record.analysisResult.treatmentPlan;
    if (expertAdvice) {
        if (currentY > 240) doc.addPage();
        currentY = addSectionTitle(doc, "5. Lời khuyên từ Chuyên gia", currentY > 240 ? 42 : currentY);

        if (expertAdvice.recommendations && expertAdvice.recommendations.length > 0) {
            doc.setFont('BeVietnamPro', 'bold');
            doc.setFontSize(11);
            doc.text("Khuyến nghị chung:", PAGE_MARGIN, currentY);
            currentY += 6;
            
            expertAdvice.recommendations.forEach(item => {
                const itemHeight = addBulletedStyledText(doc, item, PAGE_MARGIN + 4, currentY, 178);
                if (currentY + itemHeight > 270) {
                    doc.addPage();
                    currentY = 42;
        ...
        if (expertAdvice.combinedTreatments && expertAdvice.combinedTreatments.length > 0) {
            doc.setFont('BeVietnamPro', 'bold');
            doc.setFontSize(11);
            doc.text("Liệu pháp kết hợp/Thuốc uống:", PAGE_MARGIN, currentY);
            currentY += 6;
            
            expertAdvice.combinedTreatments.forEach(item => {
                const itemHeight = addBulletedStyledText(doc, item, PAGE_MARGIN + 4, currentY, 178);
                 if (currentY + itemHeight > 270) {
                    doc.addPage();
                    currentY = 42;
...
// FIX: Added generateMelasmaReportPDF function
export const generateMelasmaReportPDF = async (record: Omit<MelasmaPatientRecord, 'id' | 'createdAt'>): Promise<void> => {
    const doc = setupDoc();
    let currentY = 0;
    ...
    doc.setFont('BeVietnamPro', 'bold');
    doc.setFontSize(20);
    doc.text("PHÁC ĐỒ ĐIỀU TRỊ NÁM", 105, currentY, { align: 'center' });
    currentY += 15;
    ...
    // --- Melasma Info ---
    currentY = addSectionTitle(doc, "2. Thông tin Tình trạng Nám", currentY);
    const melasmaInfoData = [
        { label: 'Loại nám:', value: record.melasmaInfo.melasmaType },
        { label: 'Vị trí:', value: record.melasmaInfo.location },
        { label: 'Thời gian bị nám:', value: record.melasmaInfo.duration },
        { label: 'Yếu tố khởi phát:', value: record.melasmaInfo.triggers },
        { label: 'Điều trị trước đây:', value: record.melasmaInfo.pastTreatments },
    ];
    melasmaInfoData.forEach(info => {
        doc.setFont('BeVietnamPro', 'bold');
        doc.text(info.label, PAGE_MARGIN, currentY);
        doc.setFont('BeVietnamPro', 'normal');
        const valueLines = doc.splitTextToSize(info.value, 140);
    ...
    if (record.melasmaInfo.notes) {
        doc.setFont('BeVietnamPro', 'bold');
        doc.text("Ghi chú:", PAGE_MARGIN, currentY);
        doc.setFont('BeVietnamPro', 'normal');
        const notesLines = doc.splitTextToSize(record.melasmaInfo.notes, 140);
        doc.text(notesLines, PAGE_MARGIN + 40, currentY);
        currentY += notesLines.length * 5 + 3;
    }
    currentY += 5;
    ...
    // --- Treatment Plan ---
    currentY = addSectionTitle(doc, "4. Phác đồ điều trị Nám", currentY);
    currentY -= 5;
    
    const tableBody: any[] = [];
    const { inClinicProcedures, homeCareRoutine, sunProtectionAdvice } = record.analysisResult.treatmentPlan;
    ...
    if (sunProtectionAdvice.length > 0) {
        tableBody.push([{ content: 'Tư vấn chống nắng', rowSpan: sunProtectionAdvice.length, styles: { valign: 'middle', halign: 'center', font: 'BeVietnamPro', fontStyle: 'bold' } }, sunProtectionAdvice[0]]);
        for (let i = 1; i < sunProtectionAdvice.length; i++) tableBody.push([sunProtectionAdvice[i]]);
    }
    ...
    if (expertAdvice) {
        ...
        currentY = addSectionTitle(doc, "5. Lời khuyên từ Chuyên gia", currentY);
    ...
    const safeFullName = record.patientInfo.fullName.replace(/[^a-zA-Z0-9]/g, '_') || 'BenhNhan';
    const date = new Date().toISOString().slice(0, 10);
    doc.save(`PhacDoNam_${safeFullName}_${date}.pdf`);
};
